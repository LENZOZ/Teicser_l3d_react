...
export const ApiTable = <T extends unknown = any>(props: Props<T>) => {
  ...
  const columnsMemo = useMemo(() => {
    return columnsWithSortableMemo.map((column) => {
      ...
      function titleCellFunction(row: T, rowIndex: number) {
        ...
        return (
          <label style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={selectorResult}>
            {cadena}
          </label>
        );
      }
  ...
  const handleFecthData = async () => {
    ...
    if (typeof props.source === 'string') {
      let params = props.paginationServe ? getParamsToPaginateServer() : props.queryParams;
